    ...
    <div className="min-h-screen bg-[#f5f4f7] text-[#111827]">
      <div className="relative min-h-screen bg-[radial-gradient(circle_at_50%_0%,rgba(255,255,255,0.98),rgba(244,244,247,0.96)_48%,#eceff4_100%)]">
        ...
        <div className="mx-auto flex min-h-screen max-w-[1260px] flex-col px-3 py-4 sm:px-5 sm:py-6">
          <div className="relative flex min-h-[calc(100vh-2rem)] flex-col overflow-hidden rounded-[34px] border border-[#e9edf3] bg-[linear-gradient(180deg,rgba(251,251,253,0.95)_0%,rgba(245,247,251,0.96)_100%)] shadow-[0_40px_110px_-74px_rgba(15,23,42,0.28)] lg:grid lg:grid-cols-[188px_minmax(0,1fr)]">
                  ...
                  {desktopNavigation.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        href={item.href}
                        className={cn(
                          "flex items-center gap-2.5 rounded-[10px] px-3 py-2.5 text-[11px] font-medium transition",
                          active
            ...
            <div className="min-w-0">
              <header className="sticky top-0 z-30 hidden border-b border-[#eceef3] bg-white/82 px-4 py-3 backdrop-blur-xl sm:px-5 lg:block lg:px-6">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h1 className="text-[17px] font-semibold tracking-[-0.05em] text-[#4b5263] lg:text-[18px]">
                      {title}
                    </h1>
              ...
              <main className="px-3 pb-24 pt-3 sm:px-5 lg:px-6 lg:pb-6">{children}</main>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="fixed inset-x-0 bottom-3 z-40 px-3 lg:hidden">
        <div className="mx-auto flex max-w-[360px] items-center justify-between rounded-[20px] border border-[#edf0f5] bg-[#f8f8fb]/96 px-1.5 py-1.5 shadow-[0_18px_46px_-30px_rgba(15,23,42,0.22)] backdrop-blur-xl">
          {navigation.map((item) => {
            const Icon = item.icon;
            const active = pathname === item.href;
            return (
              <Link
                ...
                className={cn(
                  ...
                  active ? "bg-[#eef4ff] text-[#3b82f6]" : "text-[#98a1b2]",
                )}
              >
                <Icon className="size-3.5" />
                {item.label}
